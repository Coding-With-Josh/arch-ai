          ...
          {items.map((item) => (
            item.isComingSoon ? (
              <div
                key={item.name}
                className={cn(
                  "relative flex items-center gap-3 text-xs rounded-lg px-3 py-2 bg-transparent text-zinc-400 transition-all hover:bg-transparent duration-300",
                  "dark:text-zinc-500 dark:hover:bg-transparent",
                )}
              >
              ...
              >
                ...
                {item.name}
                {/* {item.isComingSoon && (
                                  <Badge variant={"outline"} className="absolute right-2 dark:border-zinc-600">Coming Soon</Badge>
                )} */}
              </Link>
            )
          ))}
